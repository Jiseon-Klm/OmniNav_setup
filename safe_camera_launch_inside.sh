...
# ROS 2 republish 노드 사용
echo "Starting compressed image republisher..."
ros2 run image_transport republish raw compressed \
    --ros-args \
    -r in:=/camera/camera/color/image_raw \
    -r out:=/camera/camera/color/image_raw/compressed \
    2>&1 | while read line; do echo "[republish] $line"; done &

REPUBLISH_PID=$!
sleep 2

# republisher가 정상 실행중인지 확인
if ! kill -0 $REPUBLISH_PID 2>/dev/null; then
    echo "WARNING: republish node may have failed!"
    echo "Make sure ros-jazzy-compressed-image-transport is installed:"
    echo "  apt-get update && apt-get install -y ros-jazzy-compressed-image-transport"
fi
